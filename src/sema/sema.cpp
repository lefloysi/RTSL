...
StageKind stage_from_attribute(std::string_view name) {
#define RTSL_STAGE_ATTRIBUTE(attribute, stage) \
	if (name == #attribute) { return StageKind::stage; }
#include "sema/stage_attributes.def"
	return StageKind::none;
}

InterpolationKind interpolation_from_tag(std::string_view name) {
#define RTSL_STAGE_BOUNDARY_TAG(tag, interpolation) \
	if (name == #tag) { return InterpolationKind::interpolation; }
#include "sema/stage_boundary_tags.def"
	return InterpolationKind::none;
}

StageKind resolve_stage_attributes(std::span<const Attribute> attributes, std::string_view source_name, DiagnosticEngine& diagnostics) {
	StageKind stage = StageKind::none;
	for (const auto& attribute : attributes) {
		const StageKind resolved = stage_from_attribute(attribute.name);
		if (resolved == StageKind::none) {
		...
		if (stage != StageKind::none && stage != resolved) {
			diagnostics.report(DiagnosticCode::sema_unknown_name, DiagnosticSeverity::error, attribute.span.begin, source_name,
	...
	for (auto& interface : interfaces) {
		for (auto& field : interface.fields) {
			for (const auto& tag : field.tags) {
				const InterpolationKind interpolation = interpolation_from_tag(tag);
				if (interpolation == InterpolationKind::none) {
...
void check_stage_entry_signatures(const SemanticModule& module, DiagnosticEngine& diagnostics) {
	for (const auto& symbol : module.symbols) {
		if (symbol.kind != DeclKind::function || symbol.stage == StageKind::none) {
			continue;
		}
		for (const auto& parameter : symbol.parameters) {
			if (parameter.is_reference) {
...
SemanticModule Sema::analyze(const TranslationUnit& unit) {
	...
	// Built-in slots (for example `position(clip)`) do not consume a location.
	for (auto& interface : module.stage_interfaces) {
		...
		for (auto& field : interface.fields) {
			...
			if (field.interpolation == InterpolationKind::clip) {
				continue;
			}
	...
	for (const auto& decl : unit.declarations) {
		...
		const StageKind stage = decl.kind == DeclKind::function
									? resolve_stage_attributes(decl.attributes, module.source_name, diagnostics_)
									: StageKind::none;
