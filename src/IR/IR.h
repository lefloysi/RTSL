...
enum class IROp : u16 {
    ...
    FOrdLess, FOrdLessEqual, FOrdGreater, FOrdGreaterEqual,
    IEqual, INotEqual,
    SLess, SLessEqual, SGreater, SGreaterEqual,
...
struct IRFunction {
    ...
    // Source-level parameter types (one per parameter_ids slot), used together
    // with source_name to pick the right overload during call resolution.
...
struct IRModule {
    ...
    // against function source_name / parameter_type_names. Cleared once all
    // calls in the module are inlined.
