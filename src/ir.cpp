...
class FunctionLowerer {
  ...
  private:
	...
	[[nodiscard]] BuiltIn builtin_from_gl_name(std::string_view gl_name) const {
		if (gl_name == "gl_Position")
			return BuiltIn::Position;
		if (gl_name == "gl_PointSize")
			return BuiltIn::PointSize;
		if (gl_name == "gl_VertexIndex")
			return BuiltIn::VertexIndex;
		if (gl_name == "gl_InstanceIndex")
			return BuiltIn::InstanceIndex;
		if (gl_name == "gl_FragCoord")
			return BuiltIn::FragCoord;
		if (gl_name == "gl_FrontFacing")
			return BuiltIn::FrontFacing;
		if (gl_name == "gl_FragDepth")
			return BuiltIn::FragDepth;
		if (gl_name == "gl_GlobalInvocationID")
			return BuiltIn::GlobalInvocationId;
		if (gl_name == "gl_LocalInvocationID")
			return BuiltIn::LocalInvocationId;
		if (gl_name == "gl_WorkGroupID")
