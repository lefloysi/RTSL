...
#define RTSL_BIN_TRY(expr) \
	do { if (auto _rtsl_err = (expr); _rtsl_err) return _rtsl_err; } while (false)
...
// Growing writer over a caller-owned byte vector. Same context tracking as
// the reader so a failing write reports where in the payload it happened.
struct write_stream {
