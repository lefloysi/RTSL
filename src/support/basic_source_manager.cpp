...
u32 SourceManager::add_buffer(std::string_view name, std::string_view text) {
	auto& file = files_.emplace_back(SourceFile{ .name = std::string(name), .text = std::string(text) });
